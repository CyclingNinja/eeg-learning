...
REMOVE_ATTRIBUTE = [
    None
]  #'patients','sessions',None   You can use it to remove samples from the training set that have the same attribute as the test set.
N_REPETITIONS = 1
RANDOM_STATE = [87]
...
]  #'proportion' or 'folder','train_on_tuab_tueg_test_on_tueg' ,'train_on_tuab_tueg_test_on_tuab','patients','sessions'  # choose 'folder' when training and testing on tuab
...
FINAL_CONV_LENGTH = [
    "auto"
]  # The length of the input of the last convolutional layer or classification layer of the model, which can be calculate automatically by setting 'auto'
DROPOUT = [0.1]
